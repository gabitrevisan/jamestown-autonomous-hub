...
bool readJoysticks(int step) {
  ...
  // BASE — afetado pelo modo (step)
  if (jBase < JOY_DEAD_LOW)  {
    posBase = clamp(posBase - step, BASE_MIN, BASE_MAX);
    moved = true;
  }
  if (jBase > JOY_DEAD_HIGH) {
    posBase = clamp(posBase + step, BASE_MIN, BASE_MAX);
    moved = true;
  }

  // LIFT — afetado pelo modo (step)
  if (jLift < JOY_DEAD_LOW)  {
    posLift = clamp(posLift + step, LIFT_MIN, LIFT_MAX);
    moved = true;
  }
  if (jLift > JOY_DEAD_HIGH) {
    posLift = clamp(posLift - step, LIFT_MIN, LIFT_MAX);
    moved = true;
  }

  // GRIP — sempre passo fixo (NÃO afetado pelo modo)
  if (jGrip < JOY_DEAD_LOW)  {
    posGrip = clamp(posGrip - SPEED_STEP, GRIP_MIN, GRIP_MAX);
    moved = true;
  }
  if (jGrip > JOY_DEAD_HIGH) {
    posGrip = clamp(posGrip + SPEED_STEP, GRIP_MIN, GRIP_MAX);
    moved = true;
  }
...
void processSerial(String cmd) {
  ...
  if (cmd == "MODE")   {
    printEvent(preciseMode ? "PRECISE" : "SPEED");
    return;
  }
  if (cmd == "RESET")  {
    posBase = CENTER; posLift = CENTER; posGrip = CENTER;
